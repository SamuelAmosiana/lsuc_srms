...
<html lang="en" dir="ltr">
 ...
 <head>
  ...
  <title>Dashboard || Lusaka South College - Portal</title>
      ...
      <div class="logo-segment">
        ...
        <a class="flex items-center" href="{{route('login.get')}}">
          <div class="flex items-center gap-2">
            <img src="{{asset('assets/images/logo/lsuc_logo.jpeg')}}" class="black_logo h-8 w-auto" alt="logo">
            <span class="text-sm font-semibold text-gray-800">Lusaka South College</span>
          </div>
          <div class="flex items-center gap-2">
            <img src="{{asset('assets/images/logo/lsuc_logo.jpeg')}}" class="white_logo h-8 w-auto" alt="logo">
            <span class="text-sm font-semibold text-white">Lusaka South College</span>
          </div>
        ...
        <ul class="sidebar-menu">
          ...
          <li class="">
            ...
            <ul class="sidebar-submenu">
              ...
              <li>
                <a href="{{route('lecturer.admin.get_admin_permission')}}">Get Permission</a>
              </li>
          ...
          <li class="">
            ...
            <ul class="sidebar-submenu">
              <li>
                <a href="{{url('/lecturer/admin/get_students')}}">Upload Result</a>
              </li>
              <li>
                <a href="{{url('/lecturer/admin/edit_student_result')}}">Edit Result</a>
              </li>
            </ul>
  ...
  <div>
    ...
    <!-- BEGIN: Header -->
            ...
            <div class="flex justify-between items-center h-full">
              <div class="flex items-center md:space-x-4 space-x-4 rtl:space-x-reverse vertical-box">
                <a href="{{('#')}}" class="mobile-logo xl:hidden inline-block">
                  <div class="flex items-center gap-2">
                    <img src="{{asset('assets/images/logo/lsuc_logo.jpeg')}}" class="black_logo w-8 h-auto" alt="logo">
                    <span class="text-xs font-semibold text-gray-800">Lusaka South College</span>
                  </div>
                    <div class="flex items-center gap-2">
                      <img src="{{asset('assets/images/logo/lsuc_logo.jpeg')}}" class="white_logo w-8 h-auto" alt="logo">
                      <span class="text-xs font-semibold text-white">Lusaka South College</span>
                    </div>
                </a>
      ...
      dark:bg-slate-800 border dark:border-slate-900 !top-[23px] rounded-md overflow-hidden lrt:origin-top-right rtl:origin-top-left">
                ...
                right-0 top-0"></span>
                              <img src="{{asset('assets/images/admin/' . auth()->guard('admin')->user()->picture)}}" alt="{{auth()->guard('admin')->user()->name}}" class="block w-full h-full object-cover rounded-full border hover:border-white border-transparent">
                            </div>
                          </div>
                          <div class="flex-1">
      ...
      dark:bg-slate-800 border dark:border-slate-900 !top-[23px] rounded-md overflow-hidden lrt:origin-top-right rtl:origin-top-left">
                    <div class="flex items-center justify-between py-4 px-4">
                      ...
                      <a class="text-xs font-Inter font-normal underline text-slate-500 dark:text-white" href="{{route('lecturer.admin.clearance')}}">See All</a>
                    </div>
                      ...
                      <div class="flex ltr:text-left rtl:text-right relative">
                        <div class="flex-none ltr:mr-3 rtl:ml-3">
                          <div class="h-8 w-8 bg-white rounded-full">
                            {{-- <img src="{{asset('assets/images/students/' . $notifications->user->picture)}}" alt="{{$notifications->user->name}}" class="border-transparent block w-full h-full object-cover rounded-full border"> --}}
                          </div>
                        </div>
                        <div class="flex-1">
                          <a href="{{route('lecturer.admin.clearance')}}" class="text-slate-600 dark:text-slate-300 text-sm font-medium mb-1 before:w-full before:h-full before:absolute
              before:top-0 before:left-0">
              {{$notifications->user->name}}</a>
                          <div class="text-slate-600 dark:text-slate-300 text-xs leading-4">{{'Clearance'}} {{$notifications->status}}</div>
                ...
                <div class="md:block hidden w-full">
                  <button class="text-slate-800 dark:text-white focus:ring-0 focus:outline-none font-medium rounded-lg text-sm text-center
      inline-flex items-center" type="button" data-bs-toggle="dropdown" aria-expanded="false" title="{{auth()->guard('admin')->user()->name}}">
                    <div class="lg:h-8 lg:w-8 h-7 w-7 rounded-full flex-1 ltr:mr-[10px] rtl:ml-[10px]">
                      <img src="{{asset('assets/images/admin/' . auth()->guard('admin')->user()->picture)}}" alt="{{auth()->guard('admin')->user()->name}}" title="{{auth()->guard('admin')->user()->name}}" class="block w-full h-full object-cover rounded-full">
                    </div>
      ...
      overflow-hidden">
                    <ul class="py-1 text-sm text-slate-800 dark:text-slate-200">
                      ...
                      <li>
                        <a href="{{('/lecturer/admin/settings_a')}}" class="block px-4 py-2 hover:bg-slate-100 dark:hover:bg-slate-600 dark:hover:text-white font-inter text-sm text-slate-600
...
@yield('change_course_details')
@yield('change_department_details')
@yield('change_faculty_details')
@yield('change_lecturer_details')
@yield('change_level_details')
...
@yield('clearance')
@yield('dashboard')
@yield('edit_countdown')
@yield('edit_result')
@yield('edit_results')
@yield('edit_student_for_result')
@yield('edit_student_result')
@yield('get_students_for_result')
@yield('get_students')
@yield('search_course')

@yield('search_lecturer')
@yield('search_student_result')
@yield('search_student')
@yield('search_students_result')
@yield('settings_a')
...
@yield('edit_student_permission')
@yield('remove_admin_permission')
@yield('remove_lecturer_permission')
@yield('remove_student_permission')
{{-- @yield('search_students_result')
@yield('settings_a')
@yield('upload_countdown')
@yield('upload_result') --}}
  ...
  <div class="site-footer px-6 bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-300 py-4 ltr:ml-[248px] rtl:mr-[248px]">
    <div class="grid md:grid-cols-2 grid-cols-1 md:gap-5">
      <div class="text-center ltr:md:text-start rtl:md:text-right text-sm">
        ...
        Lusaka South College, All rights Reserved
      </div>
      <div class="ltr:md:text-right rtl:md:text-end text-center text-sm">
        Made by
        <a href="{{url('https://twitter.com/samuelamosiana')}}" target="_blank" class="text-primary-500 font-semibold">
          Samuel Amosiana
        </a>
      </div>
    </div>
...
<div class="bg-white bg-no-repeat custom-dropshadow footer-bg dark:bg-slate-700 flex justify-around items-center
  backdrop-filter backdrop-blur-[40px] fixed left-0 bottom-0 w-full z-[9999] bothrefm-0 py-[12px] px-4 md:hidden">
      ...
      <a href="{{('/lecturer/admin/change_picture')}}" class="relative bg-white bg-no-repeat backdrop-filter backdrop-blur-[40px] rounded-full footer-bg dark:bg-slate-700
    h-[65px] w-[65px] z-[-1] -mt-[40px] flex justify-center items-center">
        <div class="h-[50px] w-[50px] rounded-full relative left-[0px] hrefp-[0px] custom-dropshadow">
          <img src="{{asset('assets/images/admin/' . Auth::guard('admin')->user()->picture)}}" alt="{{Auth::guard('admin')->user()->name}}" title="{{Auth::guard('admin')->user()->name}}" class="w-full h-full rounded-full border-2 border-slate-100">
        </div>
      </a>
      <a href="#">
        <div>
