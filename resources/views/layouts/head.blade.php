        ...
        <ul class="sidebar-menu">
          ...
          <li class="">
            ...
            <ul class="sidebar-submenu">
              ...
              <li>
                <a href="{{route('clearance_form.get', '#clearance_status')}}">Clearance Status</a>
              </li>
            </ul>
          </li>
  ...
  <div>
    ...
    <!-- BEGIN: Header -->
          ...
          <div class="app-header z-[999] bg-white dark:bg-slate-800 shadow-sm dark:shadow-slate-700">
            <div class="flex justify-between items-center h-full">
              <div class="flex items-center md:space-x-4 space-x-4 rtl:space-x-reverse vertical-box">
                <a href="{{route('login.get')}}" class="mobile-logo xl:hidden inline-block">
                  <div class="flex items-center gap-2">
      ...
      dark:bg-slate-800 border dark:border-slate-900 !top-[23px] rounded-md overflow-hidden lrt:origin-top-right rtl:origin-top-left">
                ...
                right-0 top-0"></span>
                              <img src="{{asset('assets/images/students/' . Auth::guard('web')->user()->picture)}}" alt="{{Auth::guard('web')->user()->name}}" title="{{Auth::guard('web')->user()->name}}" class="block w-full h-full object-cover rounded-full border hover:border-white border-transparent">
                            </div>
                          </div>
                          <div class="flex-1">
                      ...
                      <div class="flex ltr:text-left rtl:text-right relative">
                        <div class="flex-none ltr:mr-3 rtl:ml-3">
                          <div class="h-8 w-8 bg-white rounded-full">
                            <img src="{{asset('assets/images/students/' . Auth::guard('web')->user()->picture)}}" alt="{{Auth::guard('web')->user()->name}}" title="{{Auth::guard('web')->user()->name}}" class="border-transparent block w-full h-full object-cover rounded-full border">
                          </div>
                        </div>
                        <div class="flex-1">
      ...
      inline-flex items-center" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                    <div class="lg:h-8 lg:w-8 h-7 w-7 rounded-full flex-1 ltr:mr-[10px] rtl:ml-[10px]">
                      <img src="{{asset('assets/images/Students/' . Auth::guard('web')->user()->picture)}}" alt="{{Auth::guard('web')->user()->name}}" title="{{Auth::guard('web')->user()->name}}" class="block w-full h-full object-cover rounded-full">
                    </div>
                    <span class="flex-none text-slate-600 dark:text-white text-sm font-normal items-center lg:flex hidden overflow-hidden text-ellipsis whitespace-nowrap">{{Auth::guard('web')->user()->name}}</span>
      ...
      overflow-hidden">
                    <ul class="py-1 text-sm text-slate-800 dark:text-slate-200">
                      ...
                      <li>
                        <a href="{{route('settings.get')}}" class="block px-4 py-2 hover:bg-slate-100 dark:hover:bg-slate-600 dark:hover:text-white font-inter text-sm text-slate-600
...
@yield('add_course_reg')
@yield('calender')

@yield('edit_clearance_form')
@yield('payment_section')
@yield('payment_preview')
@yield('payment')
@yield('print_clearance_letter')
